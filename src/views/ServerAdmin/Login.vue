  ...
  <div class="login-container">
    ...
    <div class="glass-card">
      <div class="logo-area">
        ...
        <h2>服务器监控系统</h2>
        <p>Server Monitoring Admin</p>
      </div>
      <form class="login-form" @submit.prevent="handleLogin">
        <div class="input-group">
          <label for="username">用户名 (serverAdmin)</label>
          <div class="input-wrapper">
            <input type="text" id="username" v-model="username" placeholder="请输入管理员用户名" required />
          </div>
        </div>
        <div class="input-group">
        ...
        <button type="submit" class="login-btn" :disabled="isLoading">
          <span v-if="!isLoading">监控平台登录</span>
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (response.ok && (data.code === 200 || data.code === 0 || data.success || actualToken || data.msg === 'success' || data.message === 'success')) {
      const authority = data.data ? data.data.authority : ''

      if (authority !== 'serverAdmin') {
        errorMessage.value = '权限不足，仅允许 serverAdmin 角色登录'
...
.login-background {
  ...
  background: radial-gradient(circle at 15% 50%, rgba(16, 185, 129, 0.15), transparent 25%),
    radial-gradient(circle at 85% 30%, rgba(5, 150, 105, 0.15), transparent 25%);
  z-index: 1;
}
