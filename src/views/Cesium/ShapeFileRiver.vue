    ...
    <div class="page-container">
        ...
        <div class="page-info" :class="{ 'info-collapsed': !showPageInfo }">
            <div class="info-container" v-if="showPageInfo">
                ...
                <div class="info-item">1. 加载GeoJSON河流数据</div>
...
// Utility functions ported/implemented
...
const calMidPointOnPolyline = (paths, _, totalLength) => {
    ...
    for (let i = 0; i < paths.length - 1; i++) {
        ...
        if (currentLength + segmentLength >= targetLength) {
            ...
            // Linear interpolation for approximate midpoint coords (ignoring curvature for short segments)
    ...
    return paths[Math.floor(paths.length / 2)]; // Fallback
};

const setRiverWidth = (level) => {
    return level ? level * 5 : 5; // Default width logic
};
...
const regClickPrimitive = (viewer) => {
    ...
    handler.setInputAction((click) => {
        ...
        if (Cesium.defined(pickedObject) && pickedObject.id) {
            ...
            // Can add more interaction logic here (e.g., alert or select)
        }
...
onMounted(async () => {
    ...
    // 更新点击位置的经纬度 logic is added in regClickPrimitive or a separate handler. 
    // Since regClickPrimitive is called later, let's modify regClickPrimitive first.
    // Wait, I can't modify a function that isn't in this block with replace_file_content of this block.
    // I will just add the viewer init and camera listener here. I will modify regClickPrimitive in a separate step.
    ...
    try {
        ...
        const material = new Cesium.Material({
            fabric: {
                uniforms: {
                    ...
                    image: "/images/spriteLine.png", // Using a likely available image or we need to check existence
                    speed: 2,
                },
                source,
            },
        ...
        data.features.forEach(feature => {
            ...
            const rivLevel = feature.attributes.level;
            const rivName = feature.attributes.name;
            ...
            // Label
            const totalLen = calculatePolylineLength(pls);
            const midPoint = calMidPointOnPolyline(pls, null, totalLen);
        ...
        viewer.camera.setView({
            ...
            orientation: {
                ...
                pitch: Cesium.Math.toRadians(-25.37),
                roll: 0.0,
            },
        });
