    ...
    >
      ...
      <div class="modal-header" @mousedown="startDrag">
        <div class="modal-title">
          ...
          <span class="title-text">{{ layer?.name || 'SHP 图层' }} - 属性表</span>
      ...
      <div class="modal-body">
        <div v-if="columns.length === 0 && features.length > 0" class="no-dbf-banner">
          💡 提示：只打开了 <code>.shp</code> 图形文件。如需查看 DBF 属性数据，请在打开文件时按住 <b>Ctrl</b> 或 <b>Shift</b> 键多选 <code>.shp</code> + <code>.dbf</code> 文件，或上传包含 <code>.dbf</code> 的 <code>.zip</code> 压缩包。
        </div>
  ...
  return filteredFeatures.value.map(f => {
    ...
    return {
      ...
      props: {
        ...
        '数据提示 (Note)': '建议包含 .dbf 文件一起打开以加载属性表'
      }
    }
  })
})
...
// 拖拽窗口移动 (Drag Move - 60FPS RAF Optimization)
...
// 拖拽缩放尺寸 (Resize Window - 60FPS RAF Optimization)
const startResize = (e) => {
  if (isMaximized.value) return
  resizing.value = true
...
.shp-table-modal.isMoving {
  transition: none !important;
}

/* Light Theme Styling */
.shp-table-modal.light {
  background: #ffffff;
  border: 1px solid #cbd5e1;
  color: #1e293b;
}

.shp-table-modal.light .modal-header {
  background: #f1f5f9;
  border-bottom: 1px solid #e2e8f0;
}

.shp-table-modal.light .modal-title {
  color: #0f172a;
}

.shp-table-modal.light .count-badge {
  color: #64748b;
}

.shp-table-modal.light .search-input {
  background: #ffffff;
  border: 1px solid #cbd5e1;
  color: #1e293b;
}

.shp-table-modal.light .action-btn {
  background: #e2e8f0;
  color: #475569;
}

.shp-table-modal.light .action-btn:hover {
  background: #cbd5e1;
  color: #0f172a;
}

.shp-table-modal.light .data-table th {
  background: #f8fafc;
  color: #475569;
...
.shp-table-modal.light .data-table td {
  border-bottom: 1px solid #f1f5f9;
  border-right: 1px solid #f8fafc;
  color: #334155;
}

.shp-table-modal.light .table-row:hover {
  background: #f1f5f9;
}

.shp-table-modal.light .modal-footer {
  background: #f8fafc;
  border-top: 1px solid #e2e8f0;
}

.shp-table-modal.light .page-btn {
  background: #ffffff;
  border: 1px solid #cbd5e1;
  color: #334155;
}

.shp-table-modal.light .page-btn:hover:not(:disabled) {
  background: #3b82f6;
  color: #ffffff;
  border-color: #3b82f6;
}

.shp-table-modal.light .page-select {
  background: #ffffff;
  border: 1px solid #cbd5e1;
  color: #334155;
}
...
.shp-table-modal.dark .modal-header {
  background: linear-gradient(135deg, rgba(30, 41, 59, 0.9) 0%, rgba(15, 23, 42, 0.9) 100%);
  border-bottom: 1px solid rgba(255, 255, 255, 0.1);
}

.shp-table-modal.dark .modal-title {
  color: #ffffff;
}

.shp-table-modal.dark .count-badge {
  color: rgba(255, 255, 255, 0.6);
}

.shp-table-modal.dark .search-input {
  background: rgba(30, 41, 59, 0.8);
  border: 1px solid rgba(255, 255, 255, 0.2);
  color: #f8fafc;
}

.shp-table-modal.dark .action-btn {
  background: rgba(255, 255, 255, 0.1);
  color: rgba(255, 255, 255, 0.8);
}

.shp-table-modal.dark .action-btn:hover {
  background: rgba(255, 255, 255, 0.2);
  color: #ffffff;
}

.shp-table-modal.dark .data-table th {
...
.shp-table-modal.dark .data-table td {
  border-bottom: 1px solid rgba(255, 255, 255, 0.08);
  border-right: 1px solid rgba(255, 255, 255, 0.04);
  color: rgba(255, 255, 255, 0.9);
}

.shp-table-modal.dark .table-row:hover {
  background: rgba(59, 130, 246, 0.2);
}

.shp-table-modal.dark .modal-footer {
  background: rgba(30, 41, 59, 0.8);
  border-top: 1px solid rgba(255, 255, 255, 0.1);
}

.shp-table-modal.dark .page-btn {
  background: rgba(30, 41, 59, 0.8);
  border: 1px solid rgba(255, 255, 255, 0.2);
  color: #f8fafc;
}

.shp-table-modal.dark .page-btn:hover:not(:disabled) {
  background: #3b82f6;
  color: #ffffff;
}

.shp-table-modal.dark .page-select {
  background: rgba(30, 41, 59, 0.8);
  border: 1px solid rgba(255, 255, 255, 0.2);
  color: #f8fafc;
}
...
.shp-table-modal.dark .no-dbf-banner {
